...
2. Confirm the existing composite index order is incorrect for the filter pattern.
3. Replace `idx_salary_department(salary, department)` with `idx_department_salary(department, salary)`.
4. Re-run the query and verify PostgreSQL uses the corrected index.
5. Document the findings in `Changes.md`.
